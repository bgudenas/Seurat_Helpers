...
so <- CreateSeuratObject(gene_counts, min.cells = 100)
...
sweep.res.list_so <- paramSweep_v3(so, PCs = 1:10, sct = FALSE)
...
so <- doubletFinder_v3(so, PCs = 1:10, pN = 0.25, pK = pK, nExp = nExp_poi, reuse.pANN = FALSE, sct = FALSE)
